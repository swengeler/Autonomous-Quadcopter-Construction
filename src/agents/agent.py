...
class PerimeterFollowingAgent(Agent):
    ...
    def find_attachment_site(self, environment: env.map.Map):
        ...
        if self.current_component_marker != -1:
            ...
            # aprint(self.id, "OM AND TM:")
            # print_map(tm)
            # print_map(om)
            # aprint(self.id, "HEIGHT: {}".format(self.current_structure_level))
        ...
        if simple_distance(self.geometry.position, next_position) < Agent.MOVEMENT_PER_STEP:
            ...
            if self.seed_on_perimeter:
                ...
                if self.current_block is not None \
                        and environment.block_below(self.geometry.position) is not None \
                    ...
                    if len(candidate_components) > 0:
                        ...
                        aprint(self.id, "COMPONENT: {}".format(self.current_component_marker))
            ...
            if not ret:
                # also need to check whether there is already a seed on that layer or not
