...
class PerimeterFollowingAgentGlobal(Agent):
    ...
    def __init__(self,
        ...
        super(PerimeterFollowingAgentGlobal, self).__init__(
    ...
    def recheck_task(self, environment: env.map.Map):
        changed_task = False
        self.aprint("RECHECK TASK CALLED: {}, {}".format(self.current_component_marker, self.current_task))
        ...
        if self.current_task == Task.FETCH_BLOCK or self.current_task == Task.PICK_UP_BLOCK:
            if self.current_block_type_seed:
                ...
                unfinished = self.unfinished_component_markers(environment.occupancy_map)
                self.aprint("FETCH_BLOCK THING, UNFINISHED: {}".format(unfinished))
                if self.current_component_marker not in unfinished:
                    unseeded = self.unseeded_component_markers(environment.occupancy_map)
                    if len(unfinished) != 0:
                        ...
                        return self.recheck_task(environment)
                # elif not np.array_equal(self.current_seed.grid_position,
                #                         self.component_seed_location(self.current_component_marker)) \
                #         and self.current_component_marker \
            ...
            # TODO: figure out whether something should be different for WAIT_ON_PERIMETER
            if self.current_block_type_seed:
                if environment.check_occupancy_map(self.next_seed_position):
                    # intended site for seed has been occupied, could now make a choice to attach there instead
                    unseeded = self.unseeded_component_markers(environment.occupancy_map)
                ...
                unfinished = self.unfinished_component_markers(environment.occupancy_map)
                if self.id == 2:
                    self.aprint("CHANGING TRANSPORT_BLOCK (COMP {}), UNFINISHED: {}"
                                .format(self.current_component_marker, unfinished))
                if self.current_component_marker not in unfinished:
                    if self.id == 2:
                        self.aprint("COMPONENT {} FINISHED".format(self.current_component_marker))
                    # component is finished
                    unseeded = self.unseeded_component_markers(environment.occupancy_map)
                    if len(unfinished) != 0:
                        if self.id == 2:
                            self.aprint("STILL UNFINISHED LEFT")
                        ...
                        if len(unfinished) == len(unseeded):
                            ...
                            changed_task = True
                            if self.id == 2:
                                self.aprint("ONLY UNSEEDED LEFT, RETURNING")
                        elif len(unseeded) == 0:
                            if self.id == 2:
                                self.aprint("NO UNSEEDED LEFT, SWITCHING TO: {}".format(unfinished[0]))
                            ...
                            changed_task = True
                            if self.id == 2:
                                self.aprint("CHOICE BETWEEN SEEDING AND ATTACHING, CHOOSING SEEDED {}".format(seeded[0]))
                    else:
    ...
    def transport_block(self, environment: env.map.Map):
        ...
        if self.current_path is None:
            ...
            self.aprint("TRANSPORT LEVEL: {}".format(transport_level_z))
            ...
            if not ret:
                ...
                if not self.current_block_type_seed:
                    ...
                    if self.check_component_finished(environment.occupancy_map,
                        ...
                        if self.id == 3:
                            self.aprint("Before rechecking: {}, {}, {}".format(self.current_seed.grid_position,
                                                                               self.current_component_marker,
                                                                               self.current_task))
                        self.recheck_task(environment)
                        if self.id == 3:
                            self.aprint("After rechecking: {}, {}, {}".format(self.current_seed.grid_position,
                                                                              self.current_component_marker,
                                                                              self.current_task))
                    else:
                        self.current_task = Task.MOVE_TO_PERIMETER
                    ...
                    # TODO: SHOULD STILL CHANGE THESE TO GO IN LEAST CROWDED DIRECTION
    ...
    def find_attachment_site(self, environment: env.map.Map):
        ...
        if check_map(self.hole_map, self.current_grid_position, lambda x: x > 1): # need to check if hole is closed
            ...
            if np.count_nonzero(environment.occupancy_map[self.hole_boundaries[hole_marker]] != 0) == 0:
                ...
                # TODO: should probably take shortest path/direction out instead...
        ...
        if simple_distance(self.geometry.position, next_position) <= Agent.MOVEMENT_PER_STEP:
            ...
            if not ret:
                ...
                if current_site_tuple in self.current_visited_sites:
                    # TODO: should be able to tell immediately when "trapped" in hole and "escape"
    ...
    def return_block(self, environment: env.map.Map):
        # TODO: should probably check whether the reason to return the block is still valid
        # e.g. if carrying normal block and the component to be seeded with the fetched seed has already
        # been seeded, simply turn around and go to that component (if it isn't finished)
        # e.g. if carrying seed back and layer is completed, can then go back and attach it
        ...
        if self.current_path is None:
            ...
            self.current_static_location = min_stash_location
        ...
        if simple_distance(self.geometry.position, next_position) <= Agent.MOVEMENT_PER_STEP:
            ...
            if not ret:
                ...
                if self.current_block_type_seed:
                    self.current_block.color = Block.COLORS_SEEDS[0]
                    environment.seed_stashes[self.current_static_location].append(self.current_block)
                else:
                    self.current_block.color = "#FFFFFF"
                    environment.block_stashes[self.current_static_location].append(self.current_block)
